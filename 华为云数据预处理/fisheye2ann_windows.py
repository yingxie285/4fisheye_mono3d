...
PIPELINE_SOURCE_ROOT = Path(r"C:\fisheye_2wdata")
PIPELINE_TARGET_ROOT = Path(r"C:\Users\yingxie\Desktop\mono3d-main\windows_4fisheye")
...
def run_stage_offline_fisheye_camera_only_aug(source_root: Path, target_root: Path) -> None:
    ...
    try:
        ...
        def run(
            ...
            if os.name == "nt":
                print("[offline_fisheye_camera_only_aug] Windows detected, using serial execution.")
                for scene_scan_task in scene_scan_tasks:
                    scene_dir, frame_ids, frame_decisions = _collect_scene_frame_decisions_worker(
                        scene_scan_task
                    )
            ...
            if os.name == "nt":
                for scene_task in scene_tasks:
                    scene_summary = _run_single_scene_worker(scene_task)
...
def main() -> None:
    if not PIPELINE_SOURCE_ROOT.exists():
        raise FileNotFoundError(f"PIPELINE_SOURCE_ROOT does not exist: {PIPELINE_SOURCE_ROOT}")

    source_root = _resolve_pipeline_source_root(PIPELINE_SOURCE_ROOT)
    PIPELINE_TARGET_ROOT.mkdir(parents=True, exist_ok=True)

    run_stage_offline_fisheye_camera_only_aug(source_root, PIPELINE_TARGET_ROOT)
    ...
    print(f"Input root: {source_root / 'fisheye_2wdata'}")
