...
def compute_box_3d2(dim, location, rotation_y):
    # dim: 3
    # location: 3
    # rotation_y: 1
    # return: 8 x 3
    ...
    corners_3d = np.dot(R, corners)
    corners_3d = corners_3d + np.array(location, dtype=np.float32).reshape(3, 1)
    return corners_3d.transpose(1, 0) #8个角点+1个中心点
...
def project_to_image2(pts_3d, P, img_shape):
    # pts_3d: n x 3
    # P: 3 x 4
    # return: n x 2
    h = img_shape[0]
...
def draw_box_3d(image, corners, c=(0, 0, 255)):
  ...
  for ind_f in range(3, -1, -1):
      for ind_f in range(3, -1, -1):
          ...
          if ind_f == 0:
              pt1_diag = (int(corners[f[0], 0]), int(corners[f[0], 1]))
              pt2_diag = (int(corners[f[2], 0]), int(corners[f[2], 1]))
              cv2.line(image, pt1_diag, pt2_diag, c, 1, lineType=cv2.LINE_AA)

              pt1_diag2 = (int(corners[f[1], 0]), int(corners[f[1], 1]))
              pt2_diag2 = (int(corners[f[3], 0]), int(corners[f[3], 1]))
              cv2.line(image, pt1_diag2, pt2_diag2, c, 1, lineType=cv2.LINE_AA)
  return image
