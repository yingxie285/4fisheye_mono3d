...
import os
import math
from object_det.utils.image import flip, color_aug
from object_det.utils.image import get_affine_transform, affine_transform
from object_det.utils.image import gaussian_radius, draw_umich_gaussian, draw_msra_gaussian
import pycocotools.coco as coco
...
class DddDataset(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    if self.split == 'train' and not self.opt.no_color_aug:
      color_aug(self._data_rng, inp, self._eig_val, self._eig_vec)  #0406 颜色增强
    ...
    for k in range(num_objs):
      ...
      bbox[[1, 3]] = np.clip(bbox[[1, 3]], 0, self.opt.output_h - 1)  #bbox center: array([241.4138 ,  72.44752, 263.61508,  81.69946], dtype=float32)     （已经是output空间）
      h, w = bbox[3] - bbox[1], bbox[2] - bbox[0]
      if h > 0 and w > 0:
        radius = gaussian_radius((h, w))
        radius = max(0, int(radius))
        ct = np.array(
          [(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=np.float32)  #原先，用的2D检测框中心
        # print("bbox center:", (bbox[0]+bbox[2])/2, (bbox[1]+bbox[3])/2)
        # print("uv center:", ann['uv'])
        
        # ct = np.array(ann['uv'], dtype=np.float32)  #0402 用直接保存的uv，是3D框的中心点. 会报错
        # ct = affine_transform(ct, trans_output)     #是为了把点从原图坐标映射到网络输出坐标（包含缩放+平移+下采样）
        # print("ct_bbox", ct_bbox)
        # print("ct:", ct)
